  ...
  p5Instance = new p5((sketch) => {
    ...
    function applyPinkFilter(img, pinkColor) {
      ...
      for (let y = 0; y < img.height; y++) {
        for (let x = 0; x < img.width; x++) {
          ...
          // Change all non-white pixels to pink
    ...
    function placeObjects(count, loadedImages, existingObjects=[]) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const obj = { img, x, y, targetX:x, targetY:y, rotation: sketch.random(360), targetRotation: sketch.random(360), easing: sketch.random(0.02,0.08) }
        objects.push(obj)
    ...
    sketch.setup = async () => {
      ...
      const pinkColor = sketch.color('#ffb0da') // declare pink inside setup
